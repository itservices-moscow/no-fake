 ...
 */
    ...
    (() => {
        ...
        let bodyLock = (delay = 500) => {
            let body = document.querySelector("body");
            let widthWrap = document.querySelector(".wrapper") ? document.querySelector(".wrapper").offsetWidth : window.innerWidth;
            if (bodyLockStatus) {
                let lock_padding = document.querySelectorAll("[data-lp]");
                for (let index = 0; index < lock_padding.length; index++) {
                    const el = lock_padding[index];
                    el.style.paddingRight = window.innerWidth - widthWrap + "px";
                }
                body.style.paddingRight = window.innerWidth - widthWrap + "px";
            ...
            optionAction(selectItem, originalSelect, optionItem) {
                if (originalSelect.multiple) {
                    ...
                    if (!originalSelect.hasAttribute("data-show-selected")) {
                        ...
                        selectItem.querySelectorAll(`${this.getSelectClass(this.selectClasses.classSelectOption)}`).forEach((e => {
                            e.classList.remove(this.selectClasses.classSelectOptionSelected);
                        }));
        ...
        function inputmaslFirstInit() {
            ...
            if (inputMasks.length) mhzModules.inputmask = Inputmask().mask(inputMasks);
        }
        inputmaslFirstInit();
        ...
        da.init();
        let mediaMd3 = window.matchMedia("(max-width: 767px)");
        document.addEventListener("DOMContentLoaded", (function() {
            const novosti = document.querySelector(".novosti");
            if (novosti) {
                const novostiHero = novosti.querySelector(".hero-novosti");
                if (novostiHero) {
                    novosti.setAttribute("style", `--top: ${novostiHero.offsetHeight}px`);
            ...
            if (experto) {
                ...
                if (expertImg) experto.addEventListener("change", (function() {
                    let options = experto.querySelectorAll("option");
                    let index = experto.selectedIndex;
                    expertImg.setAttribute("src", `img/${options[index].dataset.image}`);
                }));
            }
            ...
            if (menuCloseBtns.length) menuCloseBtns.forEach((menuCloseBtn => {
        ...
        function videoActions(videoContainer) {
            ...
            videoPlay.addEventListener("click", (e => {
                ...
                if (iframe) {
                    let src = iframe.src;
                    let amper = "";
                    -1 === src.indexOf("?") ? amper = "?" : amper = "&";
                    src += `${amper}autoplay=1`;
                    iframe.src = src;
                    videoPlay.hidden = true;
                }
            }));
